...
# initialize model
lpips_metric = lpips.LPIPS(net=opt.net, variant=opt.variant)
if(opt.use_gpu):
    print("gpu: "+str(opt.gpu_ids[0]))
    lpips_metric.to(device=opt.gpu_ids[0])
...
for dataset in opt.datasets:
    data_loader = dl.CreateDataLoader(dataset, dataroot=gl_conf['dataroot'], dataset_mode=opt.dataset_mode, batch_size=opt.batch_size, load_size=opt.load_size, nThreads=opt.nThreads)
    
    # evaluate model on test type
    if(opt.test_type=='twoAFC'):
        (score, results_verbose) = score_2afc_dataset(data_loader=data_loader, 
                                                func=lpips_metric.forward, 
                                                name=dataset, 
